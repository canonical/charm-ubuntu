...
lint:  ## Lint with ruff and type-check with pyright
	...
	uv run --group lint --group unit --group integration pyright
...
integration-execution:  ## Run the integration test pytest invocation directly (used inside the spread VM)
	uv run --group integration pytest --tb native --show-capture=no --log-cli-level=INFO -s --disable-warnings tests/integration $(ARGS)
